...
- **D3 (AC5 — "Parabenizar" / limitação recognize):** A superfície de E6 (`page.tsx`, `engagement-shell.tsx`, `IndividualActionSheetProps`) só reconhece `action ∈ {remind, activate}`. `page.tsx` linha 187: `params.action === "remind" || params.action === "activate" ? params.action : null` — um `?action=recognize` cai em `initialAction=null` e o Sheet não abre. Suportar reconhecimento (`top_performer_recognition`) de ponta a ponta exigiria tocar page.tsx + shell + types.ts (superfície de E4/E6), FORA da fronteira desta story. Portanto, seguindo o fallback explícito da própria story ("Ver detalhe + registro da limitação"), o menu "No ritmo" tem 3 itens funcionais: **Ver detalhe** → `/engagement?student={id}` (sem action; abre o Centro com o Histórico focado no aluno via `focusedStudentId`); **Parabenizar** → mesma navegação de Ver detalhe (com comentário no código documentando por que recognize não navega com `?action=recognize`); **Nada** → fecha o menu. Follow-up sugerido: habilitar `recognize` end-to-end em E6 (nova AC no epic).
