...
| 2026-07-09 | Fix QA (2 bugs reportados por Hugo). **BUG 1 (Sheet sem painel):** os tokens semânticos `--color-bg-*`/`--color-text-*`/`--color-border-*` viviam só em `@layer base` de `theme.css`, então o Tailwind v4 NÃO emitia as utilities (`bg-bg-card` etc. = zero regra na CSS compilada), latente app-wide mas catastrófico no painel `fixed` do Sheet (sem backdrop → transparente). Registrados via `@theme inline` (referência à var runtime de mesmo nome; light/dark preservados pela cascata). Provado por compilação: `.bg-bg-card { background-color: var(--color-bg-card) }` agora existe. Banner de erro assenta no painel sólido. **BUG 2 (escopo incoerente Sheet↔página):** `resolveEngagementScope` (helper único usado por página + todas as rotas /api/engagement/*) ignorava a lente "Vendo como", short-circuitando admin para tenant-wide (`null`) e negando aluno que a página (admin+manager sob lente Gestor) tinha no recorte. Agora honra a lente igual analytics/notifications (`resolveRoleLens`+`isManagerLens`): admin-como-admin segue tenant-wide, admin+manager-como-Gestor é escopado ao subtree via `getManagedTeamStudentIds` (não `resolveCallerStudentScope`, que tomaria o atalho de admin), manager puro inalterado. Teste novo `students-scope.test.ts` cobre admin-tenant-wide e admin-manager-lens (in-subtree retornado, out-of-subtree dropado). | Dex (@dev) |
| 2026-07-09 | **PIVÔ DE PRODUTO (Hugo, definitivo): overlay → inline.** O `IndividualActionSheet` (drawer overlay) foi REMOVIDO. Todo o fluxo individual (card do aluno, motivo, métricas, Status atual, histórico recente no Acionar, origem, preview editável, canal, envio) migrou para a nova aba **Central de Envios** (`send-center-tab.tsx`), inline na página `/engagement`. Endpoints, MessagePreviewPanel compartilhado, derivação server-side de nudgeType e a dupla trava de escopo (AC8) permanecem intactos — só mudou o HOST (aba vs. drawer). Ganho colateral: elimina a superfície `fixed` do Sheet (fecha por construção o BUG 1 de painel transparente para este fluxo). AC1 ("Sheet lateral sem perder contexto") é SUPERSEDIDO — contexto agora é preservado por ser a própria página. Arquivo `individual-action-sheet.tsx` deletado; `IndividualActionSheetProps` removido de `types.ts`; shape `EngagementStudentDetail` PROMOVIDA para `types.ts` (lacuna que o Dev Agent Record já registrava). Novo modo MANUAL: picker de aluno escopado (`GET /api/engagement/students?q=`, busca por nome DENTRO do recorte, fail-closed) + escolha de tipo de mensagem (lembrete/acionamento/reconhecimento/livre). 3 testes novos de escopo do modo busca em `students-scope.test.ts`. NOTA: a suíte não tinha teste UI dedicado do Sheet (só `derive-nudge-type.test.ts`, que continua verde); a "adaptação dos testes do Sheet" resumiu-se a estender a cobertura de escopo da rota. | Dex (@dev) |
