...
| 2026-07-20 | **Superseded (UI) por [SH-3.2](./SH-3.2.story.md):** após o Hugo testar esta tela ao vivo, o feedback ("tá muito complexo... não me faça pensar", citando Krug) motivou uma simplificação radical da UI de `meu-plano-client.tsx`. A tela e o motor de dados desta story continuam válidos; SÓ a apresentação (as 5 seções numeradas) foi redesenhada no SH-3.2 para um caminho único com plano-pronto + revelações progressivas. Fonte de dados (`page.tsx`/`study-plan-projection.ts`) e fronteira de não-persistência preservadas. | J.A.R.V.I.S. (@dev / Dex) |
