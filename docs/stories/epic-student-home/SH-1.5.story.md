...
- `apps/web/src/components/analytics/__tests__/comparison-insights-table.test.tsx` — testes AC1/AC2 (ordem+labels), AC3/AC4/AC10 (frações+degradação), AC5 (Engajamento absoluto), AC6/AC7 (Como estou + rank real, incluindo "acima da média mas não #1"), AC12; Round 2: fração nos dois lados, ranking "Xº de Y" na célula Você de Engajamento, pills sem elipse.
- `apps/web/src/components/analytics/__tests__/student-home-card.test.tsx` — Round 2: parágrafo-resumo em painel de destaque com CTA "Continuar agora" local ao painel; `NextStepBar` compartilhado intocado nas demais chamadas.
