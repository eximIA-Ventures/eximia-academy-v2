...
**JRN-E-QA-3 — RESOLVIDO em `56d94fc` (@dev, 2026-07-25).** Território tocado: `_components/builder/**` e `_components/review/**`. `_components/dashboard/**` e `lib/journey/**` NÃO foram tocados (frente paralela de QA-1/QA-2).

- **Diagnóstico confirmado, não presumido.** `journey-plan-data.ts:188` popula `progress` com `?? UNTOUCHED_MODULE_PROGRESS`, logo `progressOf` nunca devolve `null` em produção e `hasProgress` era constante `true`. Os 10 testes novos rodaram **contra o código anterior** e falharam com o DOM literal `"Você já concluiu 0 de 8 módulos: eles ficam travados, não consomem prazo e não entram no arraste"` e `"Os 0 módulos concluídos ficam travados"`.
- **O campo `hasProgress` foi REMOVIDO, não reguardado.** Trocar só a guarda nos 4 call-sites deixaria o campo vivo com um nome que não responde a pergunta que a copy faz — pegadinha armada para o próximo. Ele não tem significado útil pós-E1 e saiu de `JourneyWindow`. O tipo é confinado a `builder/` + `review/` (verificado por grep), então a remoção não atravessa território.
- **Duas guardas substituem uma.** `hasCompletedModules` (= `frozenIndices.length > 0`, exatamente a correção apontada pelo QA) guarda os 4 sites: `journey-builder.tsx` subtítulo e guia de arraste, `journey-review.tsx` subtítulo, `consequence-banner.tsx` ("o que falta"). `hasPartialProgress` (módulo NÃO concluído com `completedRatio > 0`) é achado da varredura: para quem começou e não concluiu nada, "distribuídos **por igual**" e "**Ponto de partida neutro**" também eram falsos, porque `progressAwareNeutralDurations` dá peso `1 - completedRatio` ao parcial (D4).
- **O subtítulo do construtor passou a ter os 4 estados reais**, todos alcançáveis: concluídos > 0 · prazo vencido · começou sem concluir · partida neutra de verdade.
- **Singular** (mesma varredura): `"Os 1 módulos concluídos ficam travados"` → `"O módulo concluído fica travado"`; `"eles ficam travados"` → `"ele fica travado"` com exatamente 1 concluído.
- **Cobertura:** `builder/__tests__/render.test.tsx` +10 testes cobrindo os dois caminhos — recém-matriculado (0 concluídos), começou-sem-concluir, prazo vencido sem conclusão, exatamente 1 concluído, e o padrão esparso real (0,1,2,4 concluídos, 3 vivo no meio) que continua lendo a copy de travados.
...
**Estado dos 3 itens do veredito (atualizado 2026-07-25, @dev):** (2) JRN-E-QA-3 **resolvido em
`56d94fc`**, com teste que falha contra o código anterior (§3.1). (1) JRN-E-QA-1 e JRN-E-QA-2
seguem em aberto, em território de outra frente (`lib/journey/**`, `_components/dashboard/**`).
(3) **AC-G4 continua NÃO executada** — a correção de QA-3 foi provada em jsdom, não no browser
logado, e o argumento do §5 permanece intacto: este é exatamente o defeito que um smoke de 10
segundos teria pego antes de virar defeito de QA.
