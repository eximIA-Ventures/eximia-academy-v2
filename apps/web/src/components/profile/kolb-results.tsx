...
const STYLE_CONFIG: Record<string, { icon: typeof Brain; color: string; bgColor: string }> = {
  ...
  Assimilador: { icon: Brain, color: "text-accent-blue-mid", bgColor: "bg-accent-blue-mid/15" },
  Convergente: { icon: Lightbulb, color: "text-accent-teal", bgColor: "bg-accent-teal/15" },
...
export function KolbResults({ result }: KolbResultsProps) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      {/* Main result */}
      <div className="rounded-2xl bg-gradient-to-br from-accent-teal/5 via-bg-card to-bg-card ring-1 ring-white/[0.06] p-8 text-center space-y-4">
        ...
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-accent-teal">Seu Estilo de Aprendizagem</p>
      ...
      <div className="rounded-2xl bg-bg-card ring-1 ring-white/[0.06] p-6 space-y-4">
        ...
        <div className="relative mx-auto w-full max-w-[320px] aspect-square">
          {/* Background grid */}
          <div className="absolute inset-0 rounded-xl bg-bg-primary ring-1 ring-white/[0.04]">
            {/* Axis lines */}
            ...
            <span className="absolute left-2 bottom-2 text-[9px] font-semibold text-accent-blue-mid/60">Assimilador</span>
            <span className="absolute right-2 bottom-2 text-[9px] font-semibold text-accent-teal/60">Convergente</span>
            ...
            >
              <div className={`absolute inset-1 rounded-full ${config.color === "text-accent-teal" ? "bg-accent-teal" : config.color === "text-accent-gold" ? "bg-accent-gold" : config.color === "text-accent-blue-mid" ? "bg-accent-blue-mid" : "bg-purple-500"}`} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { label: "Conceituação Abstrata", value: result.ac, max: 48, color: "bg-accent-blue-mid" },
          { label: "Experimentação Ativa", value: result.ae, max: 48, color: "bg-accent-teal" },
        ].map((mode) => (
          <div key={mode.label} className="rounded-xl bg-bg-card ring-1 ring-white/[0.06] p-4 space-y-2">
      ...
      {/* Axes */}
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-bg-card ring-1 ring-white/[0.06] p-4 text-center">
