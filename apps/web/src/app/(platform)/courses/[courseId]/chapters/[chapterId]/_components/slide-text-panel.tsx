...
const slideMarkdownComponents: Components = {
  h2: ({ children }) => (
    <h2 className="text-lg sm:text-xl font-bold text-white mt-2 mb-4 sm:mb-5 break-words">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-[15px] sm:text-base font-semibold text-white mt-6 sm:mt-8 mb-2 sm:mb-3 break-words">
      {children}
    </h3>
  ),
  p: ({ children }) => (
    <p className="text-sm sm:text-[15px] leading-[1.75] text-white/80 mb-4 sm:mb-5 break-words">
      {children}
    </p>
  ),
  ...
  ul: ({ children }) => (
    <ul className="list-disc list-outside pl-4 sm:pl-5 my-3 sm:my-4 space-y-1.5 sm:space-y-2 text-sm sm:text-[15px] text-white/80">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal list-outside pl-4 sm:pl-5 my-3 sm:my-4 space-y-1.5 sm:space-y-2 text-sm sm:text-[15px] text-white/80">
      {children}
    </ol>
  ),
...
export function SlideTextPanel({ slide, isAudioPlaying }: SlideTextPanelProps) {
  ...
  return (
    ...
    >
      <Markdown components={slideMarkdownComponents}>{slide.text_content}</Markdown>
    </div>
  )
}
