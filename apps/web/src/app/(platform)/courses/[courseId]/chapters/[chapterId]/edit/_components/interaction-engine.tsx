...
const MODES = [
  {
    ...
    icon: ClipboardList,
    color: "cerrado-600",
    desc: "Múltipla escolha, V/F, dissertativa",
  },
  {
    ...
    icon: Target,
    color: "amber-500",
    desc: "Caso real para resolução guiada",
  },
  {
    ...
    icon: FileText,
    color: "purple-500",
    desc: "Entrega avaliada com rubrica",
  },
  {
    key: "socratic_dialogue" as const,
    label: "Socrático",
    icon: MessageCircle,
    color: "varzea",
    desc: "Diálogo reflexivo com IA",
  },
]
  ...
  return (
    <div className="space-y-4">
      ...
      {currentMode === "assignment" && (
        <AssignmentBuilder assignment={currentAssignment} onChange={onAssignmentChange} />
      )}
      ...
      {!currentMode && (
        <div className="rounded-xl bg-bg-surface shadow-card p-6 text-center">
          <p className="text-sm text-text-muted">
            Selecione um modo de interação para este capítulo.
          </p>
          <p className="text-xs text-text-muted/60 mt-1">
            O aluno verá a interação ao final do conteúdo do slide/texto.
          </p>
        </div>
      )}
    </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              ...
              <button
                ...
                className="rounded-lg p-1.5 text-text-muted hover:bg-semantic-error/10 hover:text-semantic-error"
              >
                <Trash2 size={14} />
              </button>
            </div>
            ...
            <div className="space-y-2 pt-1">
              ...
              {q.question_type === "true_false" && (
                <div className="flex gap-2">
                  <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mr-2">
                    Resposta correta:
                  </p>
                  {["true", "false"].map((v) => (
                    <button
    ...
    return (
      <div className="rounded-xl bg-amber-500/5 ring-1 ring-amber-500/20 p-5 space-y-4">
        ...
        <p className="text-xs text-text-muted">
          Descreva o problema/situação e a IA gerará um cenário completo com dados, etapas guiadas e
  ...
  return (
    <div className="rounded-xl ring-1 ring-amber-500/20 bg-amber-500/5 p-4 space-y-3">
      ...
      <input
        ...
        className="w-full rounded-lg shadow-card bg-bg-primary px-3 py-2 text-sm font-semibold text-text-primary focus:border-amber-500/40 focus:outline-none"
      />
      <input
        type="text"
        value={scenario.company}
        onChange={(e) => onChange({ ...scenario, company: e.target.value })}
        placeholder="Empresa"
        className="w-full rounded-lg shadow-card bg-bg-primary px-3 py-1.5 text-xs text-text-primary focus:outline-none"
      />
      <textarea
        ...
        rows={2}
        className="w-full rounded-lg shadow-card bg-bg-primary px-3 py-1.5 text-xs text-text-primary resize-y focus:outline-none"
      />
      <textarea
        value={scenario.problem}
        onChange={(e) => onChange({ ...scenario, problem: e.target.value })}
      ...
      <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wider">
        {scenario.steps.length} etapas configuradas
      </p>
    </div>
  )
}
  ...
  if (!assignment) {
    return (
      <div className="rounded-xl bg-purple-500/5 ring-1 ring-purple-500/20 p-5 space-y-3">
        ...
        <p className="text-xs text-text-muted">
          Crie uma atividade com instruções, entregável e rubrica de avaliação.
        </p>
        <Button
          type="button"
          variant="outline"
  ...
  return (
    <div className="rounded-xl ring-1 ring-purple-500/20 bg-purple-500/5 p-4 space-y-3">
      ...
      <input
        ...
        className="w-full rounded-lg shadow-card bg-bg-primary px-3 py-2 text-sm font-semibold text-text-primary focus:border-purple-500/40 focus:outline-none"
      />
      <textarea
      ...
      <input
        ...
        onChange={(e) => onChange({ ...assignment, deliverable: e.target.value })}
      ...
      <div className="space-y-2">
        <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wider">
          Rubrica ({assignment.rubric.length} critérios)
        </p>
        {assignment.rubric.map((c, i) => (
          <div key={c.id} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 rounded-lg shadow-card bg-bg-primary px-2 py-1 text-xs text-text-primary focus:outline-none"
            />
            <input
              type="number"
              value={c.maxScore}
        ...
        <Button
          ...
          onClick={() =>
            onChange({
              ...
              rubric: [
                ...assignment.rubric,
                { id: crypto.randomUUID(), name: "", description: "", maxScore: 25 },
              ],
            })
          }
        >
