    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          open
            ? "bg-cerrado-600/15 ring-cerrado-600/30 text-cerrado-600"
            : "bg-bg-card/90 backdrop-blur-sm hover:ring-border-medium",
        )}
        aria-label="Índice"
      >
        <LayoutGrid size={16} />
      ...
      {open && (
        <div
          ref={panelRef}
          className="fixed right-3 top-14 z-30 w-[calc(100vw-1.5rem)] sm:w-72 max-h-[60vh] sm:max-h-[70vh] overflow-hidden rounded-xl bg-bg-card shadow-card shadow-[0_8px_30px_rgba(0,0,0,0.4)] animate-in fade-in zoom-in-95 duration-150 origin-top-right sm:right-6 sm:top-32 flex flex-col"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 ">
            ...
            <button
              ...
              className="flex h-6 w-6 items-center justify-center rounded-md text-text-muted hover:text-white hover:bg-bg-elevated transition-colors"
            >
              <X size={12} />
            </button>
          </div>
                  ...
                  <div key={ch.id}>
                    <Link
                      ...
                      className={cn(
                        ...
                        isCurrent
                          ? "bg-cerrado-600/10 text-white font-medium"
                    ...
                    {/* Slide sub-items */}
                    {isCurrent && hasSlides && (
                      <div className="ml-4 mt-0.5 mb-1 flex flex-col border-l border-border-subtle pl-2">
                        {slides.map((slide) => {
                      ...
                      <div className="ml-4 mt-0.5 mb-1 flex flex-col border-l border-border-subtle pl-2">
                        {currentSections.map((section) => (
                          <a
                            ...
                            href={`#${section.slug}`}
                            onClick={() => setOpen(false)}
                            className="block rounded-md px-2 py-1 text-[11px] text-text-muted transition-colors hover:text-cerrado-600 hover:bg-bg-hover"
                          >
                            {section.text}
                          </a>
                        ))}
