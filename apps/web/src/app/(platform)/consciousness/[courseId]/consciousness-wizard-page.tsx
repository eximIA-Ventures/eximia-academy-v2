...
const COURSE_CONTENT_MAP: Record<string, Partial<ConsciousnessContent>> = {
  "analise e solucao de problemas": {
    introText:
      "Resolver problemas faz parte do seu dia a dia. Mas você já parou para pensar em COMO você resolve problemas?",
    introSubtext:
      "Vamos mapear seu ponto de partida. 3 perguntas rápidas — sem resposta certa ou errada.",
    challengeQuestion:
      "Pense num problema real do seu trabalho que você tentou resolver recentemente. O que aconteceu?",
    challengePlaceholder:
      "Ex: Um equipamento quebrou e a equipe não sabia por onde começar a investigar a causa...",
    ratingQuestion:
      "Quando um problema aparece no seu trabalho, quão confiante você se sente para analisá-lo de forma estruturada?",
    ratingLabels: {
      1: "Reativo",
      2: "Intuitivo",
      3: "Estruturado",
    ...
    goalQuestion:
      "Ao final deste curso, que tipo de problema você gostaria de resolver com mais confiança?",
    goalPlaceholder:
      "Ex: Quero conseguir identificar a causa raiz de falhas de qualidade sem depender de tentativa e erro...",
...
export function ConsciousnessWizardPage({
  courseId,
  courseTitle,
  courseDescription,
}: ConsciousnessWizardPageProps) {
  ...
  const canAdvance =
    step === 0 ||
    (step === 1 && challenge.trim().length >= 20) ||
    (step === 2 && rating !== null) ||
    (step === 3 && goal.trim().length >= 20) ||
    step === 4
  ...
  function submit() {
    startTransition(async () => {
      const res = await saveConsciousnessResponse({
        courseId,
        phase: "pre",
      ...
      <div className="relative z-10 w-full max-w-[480px] overflow-hidden rounded-[28px] bg-white shadow-[0_8px_40px_rgba(0,0,0,0.18)]">
        ...
        <div className="flex justify-end px-5 pt-5">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-stone-100 text-stone-400 transition hover:bg-stone-200 hover:text-stone-600"
          >
        ...
        <div className="px-8 pb-2">
          ...
          {step === 1 && (
            <div>
              <p className="mb-1 text-[11px] font-semibold uppercase tracking-widest text-cerrado-600">
                Pergunta 1 de 3
              </p>
              <h2 className="mb-4 text-[17px] font-semibold leading-snug text-stone-900">
                {c.challengeQuestion}
              </h2>
              <textarea
                value={challenge}
              ...
              <p
                className={cn(
                  "mt-1.5 text-[11px]",
                  challenge.trim().length >= 20 ? "text-emerald-500 font-medium" : "text-stone-400",
                )}
              >
                {challenge.trim().length >= 20
                  ? "✓ Ótimo!"
                  : `${challenge.trim().length}/20 caracteres`}
              </p>
            </div>
          )}
                ...
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    className={cn(
                      ...
                      rating === n
                        ? "bg-cerrado-600 shadow-lg shadow-cerrado-600/25"
                    ...
                    <div className="min-w-0 flex-1">
                      <p
                        className={cn(
                          ...
                          rating === n ? "text-white" : "text-stone-800",
                        )}
                      >
                        {c.ratingLabels[n]}
                      </p>
                      <p
                        className={cn(
                          ...
                          rating === n ? "text-white/75" : "text-stone-400",
                        )}
                      >
                        {c.ratingDescriptions[n]}
                      </p>
                    </div>
          ...
          {step === 3 && (
            <div>
              ...
              <p
                className={cn(
                  "mt-1.5 text-[11px]",
                  goal.trim().length >= 20 ? "text-emerald-500 font-medium" : "text-stone-400",
                )}
              >
                {goal.trim().length >= 20 ? "✓ Ótimo!" : `${goal.trim().length}/20 caracteres`}
              </p>
            </div>
          )}
            ...
            <div>
              <div className="mb-5 text-center">
                ...
                <p className="mt-0.5 text-[11px] text-stone-400">
                  Revisitaremos ao final do curso.
                </p>
              </div>
              ...
              <div className="overflow-hidden rounded-2xl border border-stone-100">
                ...
                <div className="flex items-center gap-4 border-b border-stone-100 bg-cerrado-600/[0.03] px-5 py-4">
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-cerrado-600 text-lg font-bold text-white shadow-sm">
                    {rating}
                  </span>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-stone-400">
                    ...
                    <p className="text-[15px] font-semibold text-stone-800">
                      {c.ratingLabels[rating as number]}
                    </p>
                    <p className="text-[11px] text-stone-400">
                      {c.ratingDescriptions[rating as number]}
                    </p>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-2">
            {step > 0 && (
              <button
                ...
                onClick={() => go(-1)}
                disabled={isPending}
                className="rounded-full px-4 py-2.5 text-[13px] font-medium text-stone-500 transition hover:text-stone-800 disabled:opacity-40"
              >
                Voltar
              </button>
            )}
            {step < 4 ? (
              <button
                ...
                onClick={() => go(1)}
                disabled={!canAdvance}
                className="rounded-full bg-stone-900 px-6 py-2.5 text-[13px] font-semibold text-white shadow-sm transition hover:bg-stone-800 disabled:opacity-30"
              >
