import { AGORA_FIXTURE, entradaMapaFixture } from "@/components/analytics/mapa-jornada/fixture"
...
// ---------------------------------------------------------------------------
// /gauntlet-preview/mapa-jornada — harness visual DEV-ONLY da aba "Mapa da
// jornada" do Analytics do gestor.
//
// O DEFAULT É O MOTOR, e isso é a lição 2 desta série de telas escrita em
// código: `?fonte=motor` (ou nenhum parâmetro) lê o banco e monta a tela pelo
// caminho de produção; `?fonte=fixture` empurra um mundo sintético congelado
// pelo MESMO motor.
//
//   • Nos DOIS modos quem calcula é a camada `lib/analytics/mapa-jornada`. Não
//     existe caminho em que esta tela leia número escrito à mão. Na primeira
//     tela da série o preview desenhava a fixture já calculada, e por isso
//     correções existiam no código sem aparecer na tela de inspeção — o loop
//     media uma segunda implementação.
//   • O modo fixture existe só pelo determinismo: mundo congelado, screenshot
//     byte a byte reprodutível entre rodadas. O banco de produção muda sozinho.
//
// Contrato do harness (herdado das abas anteriores, e por isso REUSADO por
// import, nunca copiado): 404 em produção; tema LIGHT forçado em 3 camadas;
// largura FIXA de 1672px; moldura REAL da Academy via <PreviewShell/>.
//
// Screenshot:
//   GAUNTLET_ROTA=mapa-jornada node scripts/gauntlet-shot.mjs /tmp/gauntlet/mapa.png
//   GAUNTLET_ROTA="mapa-jornada?fonte=fixture" node scripts/gauntlet-shot.mjs ...
// ---------------------------------------------------------------------------
 ...
 *
 * `GAUNTLET_AGORA` congela o instante para o screenshot ser comparável entre
 * duas rodadas mesmo lendo o banco — "Parado há N dias" anda sozinho com o
 * relógio de parede. Sem a variável, o relógio é o de verdade, que é o certo
 * para quem abre a URL para inspecionar o estado atual.
 */
...
export default async function GauntletMapaJornadaPreviewPage({
  searchParams,
}: {
  ...
  const dados = usarFixture
    ? computeMapaJornada(entradaMapaFixture())
