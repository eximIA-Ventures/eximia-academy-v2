// ---------------------------------------------------------------------------
// O modo MOTOR do preview da "Autogestão da minha Jornada" — Tela 2 (Meus
// Padrões e Tendências). Banco real, tenant descartável, somente leitura.
// ---------------------------------------------------------------------------
// MESMA anatomia de `../autogestao-visao-geral/leitura-real.ts` e
// `../autogestao-mapa/leitura-real.ts` (mesmo tenant fixo
// "gauntlet-descartavel", mesmo par de e-mails semeados, mesma janela dupla
// de `periodoDias`) — decisão IDS: ADAPTAR por rota, não REUSAR um módulo
// compartilhado: as 3 rotas gêmeas do gestor já seguem esse mesmo padrão de
// duplicação deliberada, uma por aba, e o tipo de saída
// (`PadroesAutogestaoDados`) e o montador chamado (`montarPadroesAutogestao`)
// são próprios desta tela.
//
...
// depende.
//
// A rota real (`_autogestao/_padroes/painel.tsx`) resolve `studentId` via
// sessão autenticada (`_autogestao/recorte.ts`). Este harness não tem sessão
// — o aluno é resolvido por E-MAIL SEMEADO (`?aluno=a|b`), nunca por
// `user.id`.
//
// SOMENTE LEITURA: a única função de dado chamada é `lerFonteAutogestao` mais
// o montador puro `montarPadroesAutogestao`. Nenhuma linha aqui grava, migra
// ou semeia — a semeadura é responsabilidade exclusiva de
...
export type ResultadoAutogestao =
  | { ok: true; dados: PadroesAutogestaoDados; mapaDeCalor: ResultadoMapaDeCalor }
...
/**
 * Lê o banco e monta a Tela 2. `agoraMs` entra por PARÂMETRO — nenhuma função
 ...
 */
export async function carregarPadroesAutogestao(
  agoraMs: number,
  aluno: Aluno,
  periodoDias: 7 | 30 | 90,
  ...
  const dados = montarPadroesAutogestao(fonte, agora)
  // Mesma fonte e mesmo teto FIXO de `_autogestao/_padroes/painel.tsx`
  // (MAPA_DE_CALOR_SEMANAS_MAX, não `periodoDias`) — ver o comentário
  // correspondente lá.
  ...
  return { ok: true, dados, mapaDeCalor }
}
