...
/**
 * GET /api/analytics/autogestao/mapa — casca fina, contrato da rota.
...
const mockMontarMapaAutogestao = vi.fn()
...
vi.mock("@/lib/analytics/autogestao/montagem", () => ({
  montarMapaAutogestao: (...args: unknown[]) => mockMontarMapaAutogestao(...args),
}))
...
function buildRequest(query = ""): Request {
  return new Request(`http://localhost/api/analytics/autogestao/mapa${query}`)
}
...
describe("GET /api/analytics/autogestao/mapa", () => {
  ...
  it("falha na fonte devolve 500 com o motivo, nunca 200 com zeros", async () => {
    mockLerFonteAutogestao.mockResolvedValue({
      ...FONTE_OK,
      falhas: { ...FONTE_OK.falhas, capitulos: { codigo: "CAPITULOS", mensagem: "boom" } },
    })
    const res = await handler(buildRequest())
    expect(res.status).toBe(500)
    const body = await res.json()
    expect(body.error).toBe("boom")
    expect(mockMontarMapaAutogestao).not.toHaveBeenCalled()
  })
  ...
  it("?agora= é IGNORADO quando NODE_ENV=production", async () => {
    ...
    const agoraUsado = mockMontarMapaAutogestao.mock.calls[0]?.[1] as Date
