...
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS whitelabel_enabled BOOLEAN DEFAULT FALSE;
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS whitelabel_config JSONB DEFAULT '{}'::jsonb;
...
DROP POLICY IF EXISTS "super_admin_audit_access" ON platform_audit_log;
...
DROP POLICY IF EXISTS "super_admin_all_chapters" ON chapters;
CREATE POLICY "super_admin_all_chapters" ON chapters FOR ALL
  USING (is_super_admin()) WITH CHECK (is_super_admin());
DROP POLICY IF EXISTS "super_admin_all_questions" ON questions;
...
CREATE POLICY "super_admin_all_enrollments" ON enrollments FOR ALL
  USING (is_super_admin()) WITH CHECK (is_super_admin());
DROP POLICY IF EXISTS "super_admin_all_sessions" ON sessions;
...
CREATE POLICY "super_admin_all_messages" ON messages FOR ALL
  USING (is_super_admin()) WITH CHECK (is_super_admin());
DROP POLICY IF EXISTS "super_admin_all_analyses" ON analyses;
CREATE POLICY "super_admin_all_analyses" ON analyses FOR ALL
  USING (is_super_admin()) WITH CHECK (is_super_admin());
DROP POLICY IF EXISTS "super_admin_all_qa_reports" ON qa_reports;
